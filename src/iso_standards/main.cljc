(ns iso_standards.main
  "Kotodama WASM entrypoint for the iso_standards clean-room actor (L4) — Clojure port.
  ...
  facts (`iso_standards.<Entity>/<field>`); `*store*` is the in-memory materialization
  used by the contract test and by the WASM runtime before a live engine binds."
  (:require [clojure.string :as str]))

(def ns-prefix "iso_standards")
(def tier "L4")
...
(def entity-specs
  [{:entity "Work"      :plural "works"      :id-prefix "isostand_wor"
    :fields [:doi :title :type :publishedAt]       :required [:doi :title]
    :coerce {}                       :refs {}}
   {:entity "Author"    :plural "authors"    :id-prefix "isostand_aut"
    :fields [:orcid :givenName :familyName]        :required [:orcid :givenName]
    :coerce {}                       :refs {}}
   {:entity "Citation"  :plural "citations"  :id-prefix "isostand_cit"
    :fields [:workId :citedDoi]                    :required [:citedDoi]
    :coerce {}                       :refs {:workId "Work"}}
   {:entity "Standard"  :plural "standards"  :id-prefix "isostand_sta"
    :fields [:designation :title :organization :status] :required [:designation :title]
    :coerce {}                       :refs {}}
   {:entity "Record"    :plural "records"    :id-prefix "isostand_rec"
    :fields [:accession :database :title]          :required [:accession :database]
    :coerce {}                       :refs {}}
   {:entity "Subject"   :plural "subjects"   :id-prefix "isostand_sub"
...
(defn emit-facts
  "EAVT facts for one record: {\"iso_standards.<Entity>/<field>\" v ...}. The datomic
...
(defn healthz [] [{:status "ok" :actor "iso_standards-compat" :tier tier :entities entities} 200])
...
(defn start! [] :iso_standards-compat/ready)
